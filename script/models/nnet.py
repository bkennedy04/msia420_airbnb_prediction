...
class MyNeuralNet():
    ...
    def fit_nn(self, x_train, y_train):
        ...
        model = MLPClassifier(solver='sgd', alpha=0.0001,
                              hidden_layer_sizes=(100, 2), random_state=self.SEED)
...
if __name__=='__main__':
    ...
    binary_nn = MyNeuralNet()
    train = binary_nn.data_reader()

    # create train and test df
    x_train, x_test, y_train, y_test = binary_nn.train_test(train)

    # fit basic boosting tree
    model_nn = binary_nn.fit_nn(x_train, y_train)

    # parameter tuning -> not sure if the syntax will work for neural net
    param_grid_nn_1 = {'max_depth': [3, 5, 7, 9, 11],
                        'min_samples_leaf': [1, 3, 5, 7, 9],
                        'max_features': ['auto', 'log2', None, 0.1, 0.3, 0.5, 1],
                        'subsample': [0.1, 0.5, 1]
                        }
    model_nn2 = binary_nn.parameter_tuning(model_nn, x_train, y_train, param_grid_nn_1)
